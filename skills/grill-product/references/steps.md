...
Drive each thread to a decision or a labeled open risk. Stop at the build bar (below). Output the brief.
...
Identify the idea or problem. If nothing concrete is given, ask once for it. Treat the request as a claim to interpret.
...
Stop at the build bar. Output the brief.
...
Name what is still unresolved rather than forcing false closure.
...
- <unresolved decision or labeled assumption>
```
